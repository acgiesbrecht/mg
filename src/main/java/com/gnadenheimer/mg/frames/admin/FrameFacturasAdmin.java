...
public class FrameFacturasAdmin extends JInternalFrame {
    ...
    private void initComponents() {
        ...
        query = java.beans.Beans.isDesignTime() ? null : entityManager.createQuery("SELECT t FROM TblFacturas t WHERE EXTRACT(YEAR FROM t.fechahora) = :anoActivo ORDER BY t.fechahora");
    ...
    private void anularButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_anularButtonActionPerformed
        try {
            ...
            LOGGER.error(Thread.currentThread().getStackTrace()[1].getMethodName(), ex);
            entityManager.getTransaction().begin();
            List<com.gnadenheimer.mg.domain.TblFacturas> merged = new ArrayList<>(list.size());
            for (com.gnadenheimer.mg.domain.TblFacturas t : list) {
