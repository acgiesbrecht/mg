 ...
 * requiring any DatePickerSettings. However, the settings of a date picker can
 * optionally be customized by creating a DatePickerSettings instance and
 * passing it to the DatePicker constructor. After the date picker is
 * constructed, the settings instance should not be changed.
 *
 * By default, the language and internationalization settings of a date picker
 ...
 * the constructor of a DatePickerSettings instance, and passing that instance
 * to the constructor of a DatePicker.
 *
 * Automatic Date Validation: Every date picker stores its current text, and its
 * last valid date. The last valid date is returned when you call
 * DatePicker.getDate(). If a person uses their keyboard to type into the text
 * field, any text that is not a valid date will be displayed in red, any vetoed
 * date will have a strikethrough, and any valid date will be displayed in
 * black. Valid dates are automatically committed to the date picker. Any
 * invalid or vetoed text is automatically reverted to the last valid date
 * whenever the the date picker loses focus.
 *
 * <code>
 * // Basic usage example:
...
public class DatePicker extends JPanel implements CustomPopupCloseListener {
    ...
    /**
     * Constructor with Default Values, Create a date picker instance using the
     * default operating system locale and language, and default date picker
     * settings.
     */
    public DatePicker() {
        this(null);
     ...
     * The previous settings will be deleted. Note that calling this function
     * effectively re-initializes the picker component. All aspects of the
     * component will be changed to match the provided settings. Any currently
     * selected date will not be changed by this function.
     *
     * If the popup calendar panel is open up the time that this function is
     * called, the open calendar will not be changed by this function. Calendar
     * panels showing the new settings will be displayed only after a new popup
     * calendar panel is opened.
     */
    ...
    /**
     * addDateChangeListener, This adds a date change listener to this date
     * picker. For additional details, see the DateChangeListener class
     * documentation.
     */
    ...
    /**
     * clear, This will clear the date picker text. This will also clear the
     * last valid date.
     */
    public void clear() {
    ...
    /**
     * closePopup, This closes the calendar popup. The popup can close itself
     ...
     * moved. 4) The user may toggle the calendar with the "toggle calendar"
     * button. 5) The user may select a date in the calendar.
     */
     ...
     * for this date picker. Direct access to the calendar button is provided so
     * that the programmer can optionally assign an image to the button, or
     * perform advanced or unusual modifications to the date picker's appearance
     * and behavior.
     *
     * Note: This function should not be used to programmatically open or close
     * the date picker calendar. The following functions are provided for that
     * purpose: openPopup(), closePopup(), and togglePopup().
     */
    ...
    /**
     * getComponentDateTextField, Returns the date text field for this date
     * picker. It is not expected that most programmers will need or want to use
     * this function.
     *
     * Important Note: Direct access to the text field is provided so the
     * programmer can perform advanced or unusual modifications of the date
     ...
     *
     * The DatePickerSettings class provides multiple ways to customize the date
     * picker text field, and those functions should be preferred for the needs
     * they address. It is suggested that the programmer become familiar with
     * the functions and settings in DatePickerSettings class before directly
     * accessing the date picker text field.
     *
     * The DatePickerSettings class can customize the following text field
     * attributes: The text field width, the background and foreground colors,
     * the fonts, font size, and font colors, and the date formats.
     *
     * Warning: To ensure proper behavior of the date picker, the text field
     * should not be used to get or set the the date (or text) values of the
     * date picker. Instead, the programmer should use the DatePicker.getDate()
     * and DatePicker.getText() (and Set) functions for those purposes. The
     * DateChangeListener interface can be used to listen for changes to the
     * date value.
     */
    ...
    /**
     * getDate, This returns the last valid date, or returns null to represent
     * an empty date.
     *
     * If "DatePickerSettings.allowEmptyDates" is true, then this can return
     * null. If allow empty dates is false, then this can never return null.
     *
     * Note: If the automatic validation of date picker text has not yet
     * occurred, then the the last valid date may not always match the current
     * text.
     *
     * <pre>
    ...
    /**
     * getDateChangeListeners, This returns a new ArrayList, that contains any
     * date change listeners that are registered with this DatePicker.
     */
    ...
    /**
     * getDateStringOrEmptyString, This returns the last valid date in an
     * ISO-8601 formatted string "uuuu-MM-dd". For any CE years that are between
     * 0 and 9999 inclusive, the output will have a fixed length of 10
     * characters. Years before or after that range will output longer strings.
    ...
    /**
     * getDateStringOrSuppliedString, This returns the last valid date in an
     * ISO-8601 formatted string "uuuu-MM-dd". For any CE years that are between
     * 0 and 9999 inclusive, the output will have a fixed length of 10
     * characters. Years before or after that range will output longer strings.
     * If the last valid date is empty, this will return the value of
     * emptyDateString.
     */
    ...
    /**
     * getText, This returns the current text that is present in the date picker
     * text field. This text can contain anything that was written by the user.
     * It is specifically not guaranteed to contain a valid date. This should
     * not be used to retrieve the date picker date. Instead, use
    ...
    /**
     * isDateAllowed, This checks to see if the specified date is allowed by any
     * currently set veto policy, and allowed by the current setting of
     * allowEmptyDates.
     *
     * If allowEmptyDates is false, and the specified date is null, then this
     * returns false.
     *
     * If a veto policy exists, and the specified date is vetoed, then this
     * returns false.
     *
     * If the date is not vetoed, or if empty dates are allowed and the date is
    ...
    /**
     * isPopupOpen, This returns true if the calendar popup is open. This
     * returns false if the calendar popup is closed
     */
     ...
     * contains a parsable date or a valid empty string. Note that this does not
     * guarantee that the text in the text field is in a standard format. Valid
     * dates can be in any one of the parsingFormats that are accepted by the
     * date picker.
     *
     * More specifically, this returns true if: 1) the text field contains a
     * parsable date that exists, and that has not been vetoed by a current veto
     * policy, OR 2) (allowEmptyDates == true) and
     * dateTextField.getText().trim() contains an empty string. Otherwise
     * returns false.
     */
     ...
     * represents a "valid date" according to the settings of this date picker.
     *
     * More specifically, this returns true if: 1) the text contains a parsable
     * date that exists, and that has not been vetoed by a current veto policy,
     * OR 2) (allowEmptyDates == true) and text.trim() contains an empty string.
     ...
     *
     * If the date picker contains a lastValidDate, the calendar will be opened
     * to the month and year of the lastValidDate, with the lastValidDate marked
     * as selected.
     *
     * If the lastValidDate is null, then the calendar will be opened with
     * today's month and year displayed, and no date will be selected.
     *
     * This function creates a new calendar and a new custom popup instance each
     * time that it is called. The associated object instances are automatically
    ...
    /**
     * removeDateChangeListener, This removes the specified date change listener
     * from this date picker.
     */
    ...
    /**
     * setDate, This sets this date picker to the specified date. Dates that are
     * set from this function are processed through the same validation
     * procedures as dates that are entered by the user.
     *
     * More specifically:
     *
     ...
     * valid value".
     *
     * A date value can be checked against any current veto policy, and against
     * the allowEmptyDates setting, by calling isDateAllowed(). This can be used
     * to determine (in advance), if a particular value would be allowed.
     *
     * Note: If empty dates are allowed, and the component does not have a veto
     * policy, then all possible values will (always) be allowed. These are the
     * default settings of this component.
     *
     * Implementation Note: Whenever the text field changes to a valid date
     * string, the lastValidDate is also automatically set (unless the date is
    ...
    /**
     * setDateToToday, This sets the date to today's date. This function is
     * subject to the same validation behaviors as if a user typed today's date
     * into the date picker. See setDate() for additional details.
     */
    ...
    /**
     * setEnabled, This enables or disables the date picker. When the date
     * picker is enabled, dates can be selected by the user using any methods
     * that are allowed by the current settings. When the date picker is
     * disabled, there is no way for the user to interact with the component.
     * More specifically, dates cannot be selected with the mouse, or with the
     * keyboard, and the date picker components change their color scheme to
     * indicate the disabled state. Any currently stored text and last valid
     * date values are retained while the component is disabled.
     */
    @Override
    ...
    /**
     * setLocale, The locale for a DatePicker should generally be set in the
     * DatePickerSettings. This function only exists to avoid confusion with the
     * swing function Component.setLocale().
     *
     * This forwards any function calls to:
     * DatePicker.getSettings().setLocale(). For the complete Javadocs, see
     * DatePickerSettings.setLocale().
     */
    @Override
    ...
    /**
     * setText, This sets the text of the date picker text field to the supplied
     * value. This will have the same effect on the last valid date as if the
     * user was typing into the text field. In other words, it may or may not
     * change the last valid date. This should not be used to set the date of
    ...
    /**
     * toString, This returns the last valid date in an ISO-8601 formatted
     * string "uuuu-MM-dd". For any CE years that are between 0 and 9999
     * inclusive, the output will have a fixed length of 10 characters. Years
     * before or after that range will output longer strings. If the last valid
     * date is empty, this will return an empty string (""). This returns the
     * same value as getDateStringOrEmptyString().
     */
    @Override
    ...
    /**
     * togglePopup, This creates and shows a calendar popup. If the popup is
     * already open, then this will close the popup.
     *
     * This is called when the user clicks on the toggle calendar button of the
     * date picker. This function does not generally need to be called
     * programmatically.
     */
    ...
    /**
     * zAddTextChangeListener, This add a text change listener to the date text
     ...
     * preferred) width of the date picker text field. The width will be
     * determined (or calculated) from the current date picker settings, as
     * described below.
     *
     * If the programmer has not supplied a setting for the minimum size, then a
     * default minimum size will be calculated from the AD date format, the
     * locale, and the font for valid dates.
     *
     ...
     * will set the text to match the "last valid date" in a standardized valid
     * format. This function is automatically called whenever the date picker
     ...
     * This will replace the invalid text in the text field with a standard date
     * field text string that matches the last valid date.
     */
    ...
    /**
     * zGetStandardTextFieldDateString, This returns a string for the supplied
     * date (or null), in the standard format which could be used for displaying
     * that date in the text field.
     */
    ...
    /**
     * zInternalSetDateTextField, This is called whenever we need to
     * programmatically change the date text field. The purpose of this function
     ...
     * variable lastValidDate. Otherwise, the lastValidDate will not be changed.
     *
     * This will also call the function to indicate to the user if the currently
     * text is a valid date, invalid text, or a vetoed date. These indications
    ...
    private void zEventTextFieldChanged() {
        ...
        zDrawTextFieldIndicators();
        //pcs.firePropertyChange("date", parsedDate, parsedDate);
    }
     ...
     * instead of a JFormattedTextField as its text input component, because a
     * date-formatted JFormattedTextField stores its value as a java.util.Date
     * (Date) object, which is not ideal for this project. Date objects
     * represent a specific instant in time instead of a "local date" value.
     * Date objects also require time zone calculations to convert them to a
     * java.time.LocalDate. This class displays and stores "local dates" only,
     * and is designed to function independently of any time zone differences.
     * See java.time.LocalDate for details on the meaning of a LocalDate. To
     * gain the validation functionality of a JFormattedTextField, this class
     * implements a similar "commit" and "revert" capability as the
     ...
     * CustomPopup that is associated with this date picker is closed. This
